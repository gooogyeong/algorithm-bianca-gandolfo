...
// while loop #1
Tree.prototype.traverseBreadthFirst = function (fn) {
  const queue = [this] // Node(2)
  while (queue.length) {
    const node = queue.shift() // Node(2)
    fn(node.value)
    node.children.forEach((child) => {
      queue.push(child) // queue = [Node(2), Node(7), Node(9)]
    })
  }
}
...
// while loop #2
Tree.prototype.traverseBreadthFirst = function (fn) {
  const queue = [this] // [Node(22), Node(7), Node(9)]
  while (queue.length) {
    const node = queue.shift() // Node(22)
    fn(node.value)
    node.children.forEach((child) => {
      queue.push(child) // queue = [Node(7), Node(9), // Node(11), Node(20)]
    })
  }
}
...
// while loop #3
Tree.prototype.traverseBreadthFirst = function (fn) {
  const queue = [this] // [Node(7), Node(9), // Node(11), Node(20)]
  while (queue.length) {
    const node = queue.shift() // Node(7)
    fn(node.value)
    node.children.forEach((child) => {
      queue.push(child) // queue = [Node(9), // Node(11), Node(20) / Node(99), Node(10)]
    })
  }
}
*/
